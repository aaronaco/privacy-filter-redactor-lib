...
from typing import List, Optional, Union, Callable, Tuple, Final
from transformers import AutoTokenizer, AutoModelForTokenClassification
from .entities import DetectedEntity
from .decoder import ViterbiCRFDecoder, DecodingMode, BIOTag
...
class PIIRedactor:
    ...
    def __init__(
        ...
        self.decoders = {
            mode: ViterbiCRFDecoder(self.model.config.id2label, mode=mode)
            for mode in DecodingMode
        }

    def _process(
        self, text: str, mode: Optional[DecodingMode] = None
    ) -> List[DetectedEntity]:
        """
    ...
    def _map_indices_to_entities(
        ...
        for i, label_id in enumerate(label_indices):
            ...
            if tag_part == BIOTag.SINGLE:
                start, end = self._trim_whitespace_from_offsets(original_text, int(start_char), int(end_char))
                detected_entities.append(DetectedEntity(
                    label=category_name, start=start, end=end, text=original_text[start:end], score=token_score
                ))
                partial_entity_metadata = None
            ...
            elif tag_part == BIOTag.INSIDE and partial_entity_metadata and category_name == partial_entity_metadata["label"]:
                partial_entity_metadata["end"] = int(end_char)
                partial_entity_metadata["token_scores"].append(token_score)
                
            elif tag_part == BIOTag.END and partial_entity_metadata and category_name == partial_entity_metadata["label"]:
                partial_entity_metadata["end"] = int(end_char)
                partial_entity_metadata["token_scores"].append(token_score)
                ...
                mean_confidence = sum(partial_entity_metadata["token_scores"]) / len(partial_entity_metadata["token_scores"])
    ...
    def detect(self, text: str, mode: Optional[DecodingMode] = None) -> List[DetectedEntity]:
        """
    ...
    def redact(
        self,
        text: str,
        mode: Optional[DecodingMode] = None,
        placeholder: Optional[Union[str, Callable[[str, str], str]]] = None,
    ) -> str:
        """
    ...
    def _apply_redaction_to_text(
        ...
        original_text: str,
        detected_entities: List[DetectedEntity],
        placeholder_logic: Optional[Union[str, Callable[[str, str], str]]] = None,
    ) -> str:
        """
