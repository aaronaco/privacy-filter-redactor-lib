...
EXAMPLES = {
    "System Log (Short)": (
        "SYSTEM LOG: Error in worker node-7. Failed to authenticate using API key 'sk-live-51MzW2EJfGq8'. Action initiated by admin@tech-startup.io.",
        DecodingMode.BALANCED.value
    ),
    "User Prompt (Short)": (
        "USER PROMPT: Hey, I'm Alice Smith from 123 Maple Ave. My social is 555-01-9999. Can you help me find my tax records from 2025?",
        DecodingMode.HIGH_RECALL.value
    ),
    "Agent Output (Short)": (
        "AGENT OUTPUT: I've successfully processed the invoice for John Doe. Please call him at (555) 012-3456 to confirm receipt of the $1,200 payment.",
    ...
    "Customer Support Email (Long)": (
        ...
        "and I recently moved from 789 Broadway, Vancouver, BC to 456 Oak Street, Seattle, WA 98101. "
...
def process_text(text, mode):
    ...
    redacted_text, entities = redactor.redact_with_details(
        text, 
        mode=DecodingMode(mode.lower())
    )
    
    entity_data = [
        [ent.label, ent.text, f"{ent.start}:{ent.end}", f"{ent.score:.4f}"] 
        for ent in entities
    ]
...
with gr.Blocks(title="Privacy Filter Redactor") as demo:
    ...
    example_dropdown.change(
        ...
        outputs=[input_text, mode_dropdown]
    )

    submit_btn.click(
        fn=process_text,
        inputs=[input_text, mode_dropdown],
        outputs=[output_text, entity_table]
    )
